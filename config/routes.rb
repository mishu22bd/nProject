...
#  match 'http://localhost:3000/homes',  :to => 'welcome#index', :as => 'home'
  ...
  resources :projects do
    ...
    resources :files, :only => [:index, :new, :create, :update]
    match 'files/changeDescription', :controller => 'files', :action => 'changeDescription', :via =>[:get,:put], :as => 'changeDescription'
    match 'files/visibility', :controller => 'files', :action => 'visibility', :via =>[:get,:put], :as => 'visibility'
    ...
    get 'task', :to => 'issues#tasks'
    get 'specials', :to => 'issues#dash_pages'
    
    resources :news, :except => [:show, :edit, :update, :destroy]
    resources :time_entries, :controller => 'timelog' do
      get 'report', :on => :collection
    end
    resources :queries, :only => [:new, :post,:create]
    shallow do
