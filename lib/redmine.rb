...
Redmine::AccessControl.map do |map|
  ...
  map.project_module :issue_tracking do |map|
    # Issue categories
    map.permission :manage_categories, {:projects => :settings, :issue_categories => [:index, :show, :tasks, :dash_pages, :new, :create, :edit, :update, :destroy]}, :require => :member
    # Issues
    map.permission :view_issues, {:issues => [:index, :show],
                                  ...
                                  :read => true
    map.permission :view_tasks, {:issues => [:tasks, :show],
                                  :auto_complete => [:issues],
                                  :context_menus => [:issues],
                                  :versions => [:tasks, :show, :status_by],
                                  :journals => [:tasks, :diff],
                                  :queries => :tasks,
                                  :reports => [:issue_report, :issue_report_details]},
                                  :read => true
    #map.permission :view_tasks ,{:issues => [:tasks]}
    map.permission :view_dash_pages ,{:issues => [:dash_pages]}
...
Redmine::MenuManager.map :top_menu do |menu|
  menu.push :home, :home_path, :caption => 'Dashboard'
  ...
  #menu.push :issues, {:controller => 'issues', :action => 'index'}, :caption => "Issues",:if => Proc.new { User.current.logged? } 
  #menu.push :files, {:controller => 'fileviews', :action => 'index'}, :caption => "File",:if => Proc.new { User.current.logged? } 
...
Redmine::MenuManager.map :account_menu do |menu|
  ...
  menu.push :my_link, { :controller => 'my', :action => 'account' }, :caption => '', :if => Proc.new { User.current.logged? }
  menu.push :logout, :signout_path, :html => {:method => 'post'}, :caption => '', :if => Proc.new { User.current.logged? }
end
...
Redmine::MenuManager.map :admin_menu do |menu|
  ...
  menu.push :trackers, {:controller => 'trackers'}, :caption => :label_tracker_plural, :if => Proc.new { User.current.login == "admin" }
  menu.push :issue_statuses, {:controller => 'issue_statuses'}, :caption => :label_issue_status_plural,
            :html => {:class => 'issue_statuses'}, :if => Proc.new { User.current.login == "admin" }
  menu.push :workflows, {:controller => 'workflows', :action => 'edit'}, :caption => :label_workflow, :if => Proc.new { User.current.login == "admin" }
  menu.push :custom_fields, {:controller => 'custom_fields'},  :caption => :label_custom_field_plural,
            :html => {:class => 'custom_fields'},:if => Proc.new { User.current.login == "admin" }
  menu.push :enumerations, {:controller => 'enumerations'}, :if => Proc.new { User.current.login == "admin" }
  menu.push :settings, {:controller => 'settings'}, :if => Proc.new { User.current.login == "admin" }
  menu.push :ldap_authentication, {:controller => 'auth_sources', :action => 'index'},
            :html => {:class => 'server_authentication'}, :if => Proc.new { User.current.login == "admin" }
  menu.push :plugins, {:controller => 'admin', :action => 'plugins'}, :if => Proc.new { User.current.login == "admin" }, :last => true
  menu.push :info, {:controller => 'admin', :action => 'info'}, :caption => :label_information_plural, :if => Proc.new { User.current.login == "admin" }, :last => true
end
