...
class IssuesController < ApplicationController
  ...
  menu_item :tasklist, :only => [:tasks]
  menu_item :special, :only => [:dash_pages]
  default_search_scope :issues

  before_filter :find_issue, :only => [:show, :edit, :update]
  before_filter :find_issues, :only => [:bulk_edit, :bulk_update, :destroy]
  before_filter :find_project, :only => [:new, :create, :update_form]
  before_filter :authorize, :except => [:index, :tasks, :dash_pages]
  before_filter :find_optional_project, :only => [:index, :tasks, :dash_pages]
  before_filter :check_for_default_issue_status, :only => [:new, :create]
  before_filter :build_new_issue_from_params, :only => [:new, :create, :update_form]
  accept_rss_auth :index, :tasks, :show
  accept_api_auth :index, :tasks, :show, :create, :update, :destroy
  ...
  def index
    ...
    if @query.valid?
     ...
     # @params_page =  params['page']
      ...
      #@issues = @issues.where(:tracker_id => 4)
      @issue_count_by_group = @query.issue_count_by_group
      
      @issues_only =  @issues.reject{| v| v.tracker.to_s == 'Task'}
      @issue_count_only = @issues_only.count
      @issue_pages_only = Paginator.new @issue_count_only, @limit, params['page']
      @offset ||= @issue_pages_only.offset
      #@issues = @issues.where(:tracker_id => 4)
      @issue_count_by_group = @query.issue_count_by_group
...
def dash_pages
    ...
    #@tasks = Issue.where(:tracker_id => 4)
     @specials = @query.issues(:include => [:assigned_to, :tracker, :priority, :category, :fixed_version],
                              :order => sort_clause,
                              :offset => @offset,
                              :limit => @limit
                              )
...
#= lol 
      ...
      Rails.logger.info "Debug specials end"
